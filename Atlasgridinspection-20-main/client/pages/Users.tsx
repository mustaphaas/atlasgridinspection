...
export default function Users() {
  ...
  const filtered = useMemo(() => users.filter((user) => {
    ...
    const searchable = `${user.name} ${user.email ?? ""} ${user.phone ?? ""} ${user.username} ${user.organization}`.toLowerCase();
    return (!query || searchable.includes(query)) && (role === "All roles" || user.role === role);
  }), [role, search, users]);

  const submit = () => {
    const email = draft.email?.trim().toLowerCase() ?? "";
    if (!draft.name.trim() || !email) {
      setNotice("Name and email are required for REA, reviewer, consultant-admin and audit accounts.");
      return;
    }
    addUser({ ...draft, email, username: email, credential: draft.credential || "ChangeMe123" });
    setDraft(emptyUser);
    setShowAdd(false);
  ...
  return (
    <section className="ag-page ag-users-page">
      <PageTitle
        ...
        title="Users"
        description="Manage REA reviewers, consultant administrators and audit access. Field-officer accounts are created by their consultant organizations and remain visible here for oversight."
        ...
        actions={<button className="ag-button ag-button-primary" onClick={() => setShowAdd(true)}><UserPlus size={16} /> Add portal user</button>}
      />
      ...
      <Panel title="User directory" subtitle={`${filtered.length} users shown`} action={<div className="ag-inline-filters"><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search users, email or phone" /></label><select value={role} onChange={(event) => setRole(event.target.value)}><option>All roles</option><option>REA Admin</option><option>REA Reviewer</option><option>Consultant Admin</option><option>Field Officer</option><option>Auditor</option></select></div>}>
        <div className="ag-table-scroll">
          <table className="ag-table">
            ...
            <tbody>
              {filtered.map((user) => (
                <tr key={user.id}>
                  <td><div className="ag-person"><span>{user.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span><div><b>{user.name}</b><small>{user.role === "Field Officer" ? user.phone ?? user.username : user.email ?? user.username}</small></div></div></td>
                  <td><b>{user.role}</b><small>{user.id}</small></td>
                  <td>{user.organization}</td>
                  <td>{user.state}</td>
                  <td>{user.lastActive}</td>
                  <td><StatusBadge status={user.status} /></td>
                  <td>{user.role === "Field Officer" ? <span className="ag-muted">Consultant managed</span> : <button className="ag-table-action" onClick={() => { toggleUserStatus(user.id); setNotice(`${user.name} access status updated.`); }}>{user.status === "Suspended" ? "Reactivate" : "Suspend"}</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showAdd && (
        <Modal title="Add portal user" subtitle="Create an REA, consultant-admin or audit account. Field officers are created inside the Consultant Portal." onClose={() => setShowAdd(false)}>
          <div className="ag-form-grid">
            ...
            <label>Email / username<input type="email" value={draft.email ?? ""} onChange={(event) => setDraft({ ...draft, email: event.target.value, username: event.target.value.toLowerCase() })} placeholder="name@example.com" /></label>
            <label>Role<select value={draft.role} onChange={(event) => setDraft({ ...draft, role: event.target.value as PortalUser["role"] })}><option>REA Admin</option><option>REA Reviewer</option><option>Consultant Admin</option><option>Auditor</option></select></label>
            ...
            <label>Temporary password<input type="text" value={draft.credential} onChange={(event) => setDraft({ ...draft, credential: event.target.value })} /></label>
